...
//20251011 DC6AP  add ansi support
//20251011 DC6AP  add remove_all_dots of a string
      ...
      while ((di = readdir(d)) != NULL)
      {
        if (stristr(di->d_name, "messages"))
        {
          ...
          if (pkt && stricmp(pkt + 1, "BAK") && isalpha(pkt[1]))
          {
            ...
            remove_dots(msg[sprachen].msgkenner);
            found = 0;
            for (k = 0; k < j; k++)
            {
...
void remove_dots(char *str)
{
    ...
    *dst = '\0';  // String korrekt terminieren
}
...
//*************************************************************************
{
  ...
  if (u->ansicolor)
    trace(report, "msg_load", "ansi %s: %d lines", mm->msgkenner, msgnum);
  else
...
//*************************************************************************
{
  ...
  if (b->msg_loadnum != msg_loadnum)
  {
    ...
    if (! msg[offset].msgpointer)
    {
      trace(report, "ms", "ansicolor: %d ", u->ansicolor);
      msg_load(u->sprache);
    }
    b->msg_offset = offset;
