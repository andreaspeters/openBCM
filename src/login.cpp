...
//20251011 DC6AP  ADD readtext for ans files
...
static char *msgpath (char *name, char *land, char *ans)
//*************************************************************************
//
//  Setzt einen Filepfad fuer ein sprachabhaengiges Textfile zusammen
//
...
//*************************************************************************
{
  ...
  int prozent = 0;
  FILE *f;
  char filename[FNAMELEN+10];
  
  if (u->ansicolor)
  {
    f = s_fopen(msgpath(text, ms(m_helpext), "ANS"), "lrt");
    if (!f) f = s_fopen(msgpath(text, "GB", "ANS"), "lrt");
    if (!f) f = s_fopen(msgpath(text, "DL", "ANS"), "lrt");
  } else {
    // Fallback auf normale Dateien, falls keine .ans vorhanden
