...
//  char *z = ms(m_neverlogin);
  ...
  if (loaduser(call, &uu, 0))
  {
    ...
    if (*uu.newcall || b->sysop)
      putf(ms(m_a_newcall), *uu.newcall ? uu.newcall : "none");
    ...
    if (full)
    {
      ...
      putf(ms(m_a_psoption), optstr(uu.opt[o_ps]));
      if (strcmp(b->logincall, call) == 0 || b->sysop)
      {
        putf(ms(m_a_fhold), uu.fhold);
        putf(ms(m_a_fwddelay), uu.fdelay);
      }
      else
        putf(ms(m_a_fwddelay), uu.fdelay);
        putf(ms(m_a_userforward));
      // userforward is either off, passive or passive and active
      if (! *uu.ufwd) putf(ms(m_a_passive));
      else if (*uu.ufwd == 1) putf("%s\n", ms(m_off));
      else putf("%s\n", uu.ufwd);
      if (uu.dirformat) putf(ms(m_a_dirformat), uu.dirformat);
#ifdef __FLAT__
      if (uu.unsecure_smtp)
        putf(ms(m_a_unsecure_smtp), (uu.unsecure_smtp & 1));
#endif
      ...
      if ((m.paclen || b->sysop) && uu.paclen)
        putf(ms(m_a_paclen), uu.paclen);
#ifdef _USERCOMP
      if (uu.comp == 1)
        putf(ms(m_a_comp_active));
      else
        putf(ms(m_a_comp_not_active));
#endif
      if (strcmp(b->logincall, call) == 0 || b->sysop)
      {
        putf(ms(m_a_status), uu.status);
        putf(ms(m_a_nopurge), uu.nopurge);
        putf(ms(m_a_passwordlen),
             *uu.password == 1 ? -1 : (int) strlen(uu.password));
        putf(ms(m_a_ttypwlen), strlen(uu.ttypw));
#ifdef __LINUX__
        if (*uu.linuxpw && m.addlinuxsystemuser)
          putf(ms(m_a_linuxpwlen), strlen(uu.linuxpw));
#endif
        if (*uu.password > 1)
        {
#ifdef FEATURE_MDPW
          putf(ms(m_a_loginpwtype), pwtypestr (uu.loginpwtype));
          putf(ms(m_a_sfpwtype), pwtypestr (uu.sfpwtype));
#endif
          putf(ms(m_a_pwline), uu.pwline);
        }
        putf(ms(m_a_readlock), uu.readlock);
#ifdef FBBCHECKREAD
        putf(ms(m_a_fbbcheckmode), uu.fbbcheckmode);
#else
        putf("\n");
#endif
      }
      putf(ms(m_a_ansicolor), uu.ansicolor);
      switch (u->binmode)
      {
#ifdef FEATURE_DIDADIT
             case BINMODE_DIDADIT: putf("%-50s", ms(m_a_binmode_didat)); break;
#endif
#ifdef FEATURE_BINSPLIT
             case BINMODE_BINSPLIT: putf("%-50s", ms(m_a_binmode_binsplit)); break;
#endif
#ifdef FEATURE_YAPP
             case BINMODE_YAPP: putf("%-50s", ms(m_a_binmode_yapp)); break;
#endif
             default: putf("%-50s", ms(m_a_binmode_autobin));
      }
#ifdef __FLAT__
      putf(ms(m_a_httpsurface), uu.httpsurface);
#else
      putf("\n");
#endif
      putf(ms(m_a_away), uu.away);
      if (uu.away == 1 && uu.awayendtime)
        putf(ms(m_a_awayend), datestr(uu.awayendtime, 2));
      else
        putf("\n");
      if (uu.away == 1 && *uu.awaytext)
        putf(ms(m_a_awaymessage), uu.awaytext);
      if (*uu.notification || b->sysop)
        putf(ms(m_a_notification),
             *uu.notification ? uu.notification : ms(m_off));
      //putf("RLimit........(A RL).%d\n", uu.rlimit);
    }
    ...
    if (*uu.uplink && uu.lastboxlogin) putf("%s %s", ms(m_via), uu.uplink);
    putv(LF);
...
//****************************************************************************
{
  ...
  enum befnum
    { unsinn,
      ...
      fbbcheckmode,
#endif
      ansicolor, loginpwtype, sfpwtype, newcall, dirformat, away, qth,
      zip, awayendtime, notification,
#ifdef __LINUX__
      linuxpw,
#endif
...
//Negative lists
    ...
    switch (cmd) //list of commands not available for "tell"
    {
      ...
      case paclen: case away: case qth: case zip: case awayendtime: case ansicolor:
      case notification:
#ifdef __FLAT__
      case httpsurface:
#endif
  ...
  switch (cmd)
  {
    ...
    case ansicolor:
         {
           if (*selektor) u->ansicolor = !! atoi(selektor);
           if (u->ansicolor) {
             b->msg_loadnum = 0;
             putf(ms(m_ansicoloron));
	   }
           else
             putf(ms(m_ansicoloroff));
         } break;
    case awayendtime:
         {
           if (! u->away)
